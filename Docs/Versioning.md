...
- **Minor** (e.g. 1.0.0 -> 1.1.0): The file format changed in a way that older versions can no longer load it. A migration must be added to `Features.migrate()` in `Scenes/Features/features.gd`.
...
Once version 1.0.0 is reached, a migration must be added to `Features.migrate()` whenever the file format changes. The `version` field stored in each `.middle-earth` file identifies which application version produced it, allowing the migration function to apply the necessary transformations on load.
