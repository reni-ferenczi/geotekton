...
func test_the_samples_keep_the_edges_their_triangles_left_on_the_boundary() -> void:
	for file_name in OLD_SAMPLES:
		...
		assert_eq(str(migrated["version"]), "0.30.0", "%s is migrated to 0.30.0" % file_name)
...
func test_a_leaf_at_0_4_0_under_no_moving_group_is_left_alone() -> void:
	...
	assert_eq(migrated["version"], "0.30.0", "at the current version")
...
func test_every_sample_opens_with_one_of_the_five() -> void:
	...
	for file_name in names:
		if not file_name.ends_with(".geotekt"):
			continue
...
const SCRATCH := "user://test_migration.geotekt"
...
func _load_0_20_0(leaf: Dictionary) -> Feature:
	...
	assert_eq(Document.migrate(raw)["version"], "0.30.0", "at the current version")
	assert_eq(raw["version"], "0.20.0", "the data passed in is not changed")
	var path := SCRATCH.get_base_dir().path_join("circle_0_20_0.geotekt")
	var file := FileAccess.open(path, FileAccess.WRITE)
...
func test_a_0_22_0_ridge_becomes_a_midway_topology() -> void:
	...
	assert_eq(TopologySection.list_to_json(ridge.sections), [
		...
		{"feature": "east", "part": 0, "from": 0, "to": 2, "reversed": true, "side": 1}],
...
### 0.27.0 to 0.29.0: a leaf carries its line width


# A leaf from before has no `line_width` key, which reads as 1, the width every
# feature was drawn at. Only the version moves.
func test_a_0_27_0_file_changes_only_its_version() -> void:
	var leaf := {"type": "Feature", "title": "Fault", "feature_type": "line",
		"geometry_kind": "polyline", "rings": [[[0.0, 0.0], [0.0, 10.0]]]}
	var raw := {"version": "0.27.0", "application": "geotekt", "features": {"type": "Group",
		"title": "Root", "children": [leaf]}, "view": {"raster_path": "art/earth.png"}}
	var migrated := Document.migrate(raw)
	assert_eq(migrated["version"], "0.30.0", "at the current version")
	var expected := raw.duplicate(true)
	expected["version"] = "0.30.0"
	assert_eq(migrated, expected, "and nothing else changed")
	assert_eq(raw["version"], "0.27.0", "the data passed in is not changed")
	var feature := Feature.from_json(migrated["features"]["children"][0])
	assert_eq(feature.line_width, 1.0, "drawn at the width it always was")
	assert_eq(feature.line_scale(), 1.0, "which is the full width of a line")


# The keys a 0.29.0 file writes are the ones it reads back.
func test_a_0_29_0_file_round_trips() -> void:
	var document := Document.new()
	...
	assert_eq(document.set_line_width(circle, 2.5), "", "the circle is drawn wider")
	...
	assert_eq(Document.migrate(raw), raw, "migration leaves a 0.29.0 file as it is")
	...
	assert_eq(reloaded.root.children[0].line_width, 2.5, "the circle keeps its line width")
	assert_eq(raw["features"]["children"][0]["line_width"], 2.5, "which the file carries")
	assert_true(not (raw["features"]["children"][1] as Dictionary).has("line_width"),
		"a feature at the width it always had writes no key")
