...
func test_the_samples_keep_the_edges_their_triangles_left_on_the_boundary() -> void:
	for file_name in OLD_SAMPLES:
		...
		assert_eq(str(migrated["version"]), "0.27.0", "%s is migrated to 0.27.0" % file_name)
...
func test_a_leaf_at_0_4_0_under_no_moving_group_is_left_alone() -> void:
	...
	assert_eq(migrated["version"], "0.27.0", "at the current version")
...
func _load_0_20_0(leaf: Dictionary) -> Feature:
	...
	assert_eq(Document.migrate(raw)["version"], "0.27.0", "at the current version")
...
# The keys a 0.27.0 file writes are the ones it reads back.
func test_a_0_27_0_file_round_trips() -> void:
	var document := Document.new()
