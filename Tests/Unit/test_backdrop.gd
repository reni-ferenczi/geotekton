...
# Reading the image a document wears. The fixtures in Tests/Data/Backdrops are
...
const DIR := "res://Tests/Data/Backdrops"
...
func test_every_format_loads_with_the_picture_intact() -> void:
	for name in ["quarters.png", "quarters.jpg", "quarters.webp", "quarters.svg"]:
		var backdrop := Backdrop.load_from(_path(name))
		if not assert_no_error(backdrop, name):
			continue
...
func test_an_svg_is_rasterized_large_enough_to_wrap_a_planet() -> void:
	var backdrop := Backdrop.load_from(_path("quarters.svg"))
	if not assert_no_error(backdrop, "quarters.svg"):
		return
	assert_eq(backdrop.texture.get_width(), int(Backdrop.SVG_WIDTH),
		"the SVG is drawn at the width Backdrop asks for")


func test_an_image_that_is_not_there_says_so() -> void:
	var backdrop := Backdrop.load_from(_path("no_such_file.png"))
	assert_eq(backdrop.texture, null, "there is no texture")
	assert_true(backdrop.error.contains("not there"), "and the reason says so: %s" % backdrop.error)


func test_a_format_that_is_not_an_image_says_so() -> void:
	var backdrop := Backdrop.load_from("res://Tests/Data/craton.middle-earth")
	assert_eq(backdrop.texture, null, "there is no texture")
	assert_true(backdrop.error.contains("not an image"),
		"and the reason names the formats: %s" % backdrop.error)
...
func test_no_image_at_all_is_not_an_error() -> void:
	var backdrop := Backdrop.load_from("")
	assert_eq(backdrop.texture, null, "there is no texture")
	assert_eq(backdrop.error, "", "and nothing went wrong")


func assert_no_error(backdrop: Backdrop, name: String) -> bool:
	if backdrop.texture == null:
		fail("%s did not load: %s" % [name, backdrop.error])
		return false
	assert_eq(backdrop.error, "", "%s loaded without complaint" % name)
	return true
