...
# The shader writes linear light, so its orange is Planet.CHILD_COLOR linearized.
func test_the_shader_draws_children_in_the_same_orange() -> void:
	var numbers := _constant("CHILD_COLOR").trim_prefix("vec3(").trim_suffix(")").split(",")
	var wanted := Planet.CHILD_COLOR.srgb_to_linear()
	assert_eq(numbers.size(), 3, "the shader's CHILD_COLOR has three components")
