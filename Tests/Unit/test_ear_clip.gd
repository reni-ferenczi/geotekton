...
# Application._ear_clip triangulates a polygon given in the (latitude, longitude)
# plane and returns a flat triangle list, 3 vertices per triangle.

# A convex quad and an L shape, both wound the same way.
const QUAD: Array[Vector2] = [Vector2(0, 0), Vector2(0, 30), Vector2(30, 30), Vector2(30, 0)]
const L_SHAPE: Array[Vector2] = [
...
func test_convex_quad_yields_two_triangles() -> void:
	var result := Application._ear_clip(QUAD)
...
func test_concave_polygon_yields_four_triangles() -> void:
	var result := Application._ear_clip(L_SHAPE)
	...
	for i in range(0, result.size() - 2, 3):
		...
		var b := result[i + 1]
		var c := result[i + 2]
		var area := _signed_area([a, b, c] as Array[Vector2])
		...
		for v in L_SHAPE:
			if v == a or v == b or v == c:
				continue
			assert_true(not Application._point_in_triangle(v, a, b, c),
				"triangle %d covers the unrelated vertex %s" % [i / 3, v])


func test_degenerate_input_yields_nothing() -> void:
	assert_eq(Application._ear_clip([] as Array[Vector2]).size(), 0)
	assert_eq(Application._ear_clip([Vector2(0, 0)] as Array[Vector2]).size(), 0)
	assert_eq(Application._ear_clip([Vector2(0, 0), Vector2(10, 10)] as Array[Vector2]).size(), 0)


func test_triangulation_preserves_the_signed_area() -> void:
	for polygon in [QUAD, L_SHAPE]:
		var result := Application._ear_clip(polygon)
		var total := 0.0
		for i in range(0, result.size() - 2, 3):
			total += _signed_area([result[i], result[i + 1], result[i + 2]] as Array[Vector2])
...
func _signed_area(polygon: Array[Vector2]) -> float:
	var total := 0.0
