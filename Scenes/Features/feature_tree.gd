...
signal rule_selected(node: Feature)
...
@onready var Application := preload("res://Scenes/Application/application.gd")
...
func load_root_group(project: Project) -> void:
	clear()
	items.clear()
	self.project = project
	self.root = Feature.new("Planet", Color.AQUA)
...
func load_rule(parent: TreeItem, rule: Feature):
	var item := create_item(parent)
	assert(item != null, "ERROR: Internal error in Godot's Tree control")
	...
	item.add_button(0, Helpers.render_cell(rule.color).texture, 1, disabled, "Color")
	item.add_button(0, Helpers.get_rule_option_icon(Helpers.ICON_ENABLE + int(rule.enabled), not rule.enabled, rule.enabled), 2, false, "Enable this rule")
	item.add_button(0, Helpers.get_rule_option_icon(Helpers.ICON_INVERT, rule.invert, rule.enabled), 3, disabled, "Invert the match")
	item.add_button(0, Helpers.get_rule_option_icon(Helpers.ICON_SINGLE, rule.single, rule.enabled), 4, disabled, "Single match")
	item.add_button(0, Helpers.get_rule_option_icon(Helpers.ICON_WRAP, rule.wrap_, rule.enabled), 5, disabled, "Wrap around the edges")
	item.add_button(0, Helpers.get_rule_option_icon(Helpers.ICON_RESIZE + rule.resize, rule.resize != 0, rule.enabled), 6, disabled, "Keep the same size, crop or pad the grid")
	item.add_button(0, Helpers.get_rule_option_icon(Helpers.ICON_REPEAT, rule.repeat, rule.enabled), 7, disabled, "Repeat until there is no change")
...
func collapse_all(collapsed: bool):
	for item in items.values():
		...
		if node is Feature and not is_same(node, root):
...
func _drop_data(at_position: Vector2, data: Variant) -> void:
	...
	var from_parent := project.features[dragged_node.parent]
	if from_parent == null:
		return

	var from_index := -1 # TODO: from_parent.find_child(dragged_node)

	if over_node is Feature and drop_position != -1:
	...
	var to_parent := project.features[over_node.parent]
	if to_parent == null:
		return

	var to_index := -1 # TODO: to_parent.find_child(over_node)
	from_parent.children.remove_at(from_index)
	if drop_position == 1:
...
func on_output_clicked(node: Feature, mouse_button_index):
	if mouse_button_index == 1:
		...
		pass # TODO: Reset the color to some default
...
func on_enabled_clicked(node: Feature):
	...
	if node is Feature and not node.enabled:
		node.collapsed = true
	program_changed.emit()
